...
«Start». В самом начале база будет пустая и никакие запросы не сработают —
смотрите ниже, как её заполнить.
...
Сначала база пустая, но она не должна быть такой. На прошлом этапе вы нарисовали
на карте, как минимум, все дома в районе, и получили файл GeoJSON. Теперь давайте
зальём его в базу.

Отправьте боту команду `/admin`. Затем нажмите «База заведений», и там «Прислать файл».
...
на теги `house`, которые должны ссылаться на существующие `id`.
